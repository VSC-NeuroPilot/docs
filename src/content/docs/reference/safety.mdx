...
You can add additional Neuro-unsafe paths using the [Include Patterns setting](/docs/reference/settings#include-exclude-patterns).

You can also disable the default Neuro-unsafe path checks using the [Allow Unsafe Paths setting](/docs/reference/settings#allow-unsafe-paths).
...
You can allow Neuro to use ALL defined tasks by enabling the [Allow Running All Tasks setting](/docs/reference/permissions#non-action-permissions).
...
More info: [Settings/Terminals](/docs/reference/settings#terminals)
...
- Setting action permissions to Autopilot: Neuro will be able to execute that permission whenever she wants without needing approval. You may want to look at the [permissions reference](/docs/reference/permissions) to see what each permission group can do.
...
You can learn about the way NeuroPilot handles action permissions on the [RCE page](/docs/reference/rce).
...
As a panic button, you can activate the [Disable All Permissions command](/docs/reference/commands#disable-all-permissions). This will immediately shut off access to all permissions, even requesting cookies, and will also forcibly change certain non-action-permission settings.
