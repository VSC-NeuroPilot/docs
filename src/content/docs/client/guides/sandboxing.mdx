...
8. Open your settings, and go down to the [`NeuroPilot > Websocket Url`](/docs/client/reference/settings#websocket-url).

9. Add the host IP address and port that you spawned your Neuro API server on, in the scheme of `ws://(host):(port)`.

10. Now, [attempt to reconnect](/docs/client/reference/commands#reconnect). This should now connect to your Neuro API server as expected.
