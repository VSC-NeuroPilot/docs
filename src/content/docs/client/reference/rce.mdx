...
In this case, if Neuro tries to add files and make a commit, her permission level will be Autopilot. If Neuro wants to then push it online, her minimal permission level will be Copilot. And finally, if she wants to change where to push it to, well she can't, so she has to ask for help.
...
Additionally, actions can also have a `defaultPermission` property which overrides the `permissions` array. This is helpful for actions like `chat`, `complete_code` or `cancel_request`, which often want to run in Autopilot mode irrespective of any actual permission.
...
These validators get ran after the packet has been validated against the schema. This allows for custom validation with other tools (standout uses include Neuro-safe path checks and the Git extension's availability).
...
There are two types of validators: sync and async validators.

Sync validators are ran immediately after schema validation, and don't usually take too long to work. This allows it to fail earlier if it does fail. This was the only type of validators previously.

Async validators are ran after setting up cancellation events, whether or not you're on Autopilot.
The cancellation events ensure that async validators don't cause problems while awaiting results, and as a result are also set up on Autopilot actions too.
Async validators have a (for now hardcoded) 1-second timeout period. If timed out, the action fails execution.
...
To learn more abut Promises: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise

</Aside>

## Extra stuff

There's also some extra layers to the RCE system for each RCE action, namely:

### Prompt generator

This can either be a pre-filled string or function that takes the action data as input and spits a string back out.
It's used for generating the Copilot mode request prompt to allow Neuro to request actions be ran.

This is the only "extra" item that has actually been added at the same time as the system itself was merged into the repository.

### Cancellation events

Cancellation events can be attached to an action object to give it a trigger for when the event will be cancelled.
These use VS Code's `EventEmitter` class and `Event` function type to set up events dynamically.

To avoid wasting resources, cancellation events will be automatically disposed when the event fires or the request is accepted, as well as when the extension is deactivated.

All of these is handled through a class that contains all the components necessary to pull this off.

You can choose whether or not to enable this feature through the [Enable Cancel Events](/docs/reference/settings#enable-cancel-events).

### Preview effects

Preview effects apply a visual effect on your screen when the action is ran in Copilot mode.
This can let you see what is going to happen visually before you have to accept the request.

Most preview effects also have a tooltip that tells you what is going to happen, in addition to the Copilot request.
