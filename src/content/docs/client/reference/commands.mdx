...
## Send Selection to Neuro

Sends the currently highlighted cursor selection to Neuro.
This will only appear if something is highlighted.
This is also available as a code action quick fix (the `Ctrl` + `.` menu), with the same appearance conditions as the command.
...
Blocks, then fully disables all of Neuro's permissions, kills all currently open shells from tasks and terminals, and cancels Neuro's RCE request (if one was present).
...
Learn more about these commands at [Features/Linting analysis - Ask Neuro to fix/explain](/docs/client/reference/features/linting-analysis#ask-neuro-to-fixexplain)

## Switch current Neuro API user name

Acts as a shortcut to the [Currently As Neuro API](/docs/client/reference/settings#currently-as-neuro-api) setting, allowing you to change it to whatever value you desire.
...
## Get dependency extension states

This refreshes the state of dependency extensions, such as Copilot Chat and Git.
This also runs their API obtaining function, if there is one. As of the last edit to this section, the only extension doing this is Git.

This command is a stopgap for allowing Web builds to function, and it will be deprecated once the [planned NeuroPilot API](https://github.com/Pasu4/neuropilot/issues/45) as we will rely on companions instead having to rely on this sort-of hacky approach.

## Send Current Selection to Neuro
