...
For all other options, Neuro can choose to use either exact pattern searches or regex to search for the targeted text.
...
Allows Neuro to highlight a set of lines.
These lines will be displayed as pink highlights across the lines targeted.
This is purely for attention-grabbing. {/* TODO: for now... */}
...
Replaces the current user selection.
...
This can be either with her Get User Cursor action or by sending it via the [Send Current Selection as Context](/docs/client/reference/commands/) command.

## Diff Patch

Allows Neuro to write diff patches to modify the file.
Intended as a more direct way of modifying the file.
...
<Aside title="Support for other diff formats">

Support for more diff formats will be added later.

We are currently targeting to add Git diffs next, followed by XML diffs.
