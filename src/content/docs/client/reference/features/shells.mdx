...
banner:
    content: |
        The web release of NeuroPilot will not have this feature built-in.
---
...
Changing Neuro-safe tasks in any way requires a [task reload](/docs/client/reference/commands#reload-tasks).
...
For info about the setting, see [its entry in the settings page](/docs/client/reference/settings#terminals).
...
To prevent overwhelming Neuro, the stdout and stderrs are sent at a [configurable delay](/docs/client/references/settings#terminal-context-delay) to wait for all output before sending it to Neuro.
